...
function herd_coop_3_basic(;
  ...
  cost = let
    ...
    function stage_cost(x, u, t, context_state)
      ...
      cp1 += distance_function(xe1_evm) /3
      cp1 += distance_function(xe2_evm) /3
      cp1 += distance_function(xe3_evm) /3

      cp2 = distance_function(xp2 - evm)
      # sum of distance from evm of all evaders
      cp2 += distance_function(xe1_evm) /3
      cp2 += distance_function(xe2_evm) /3
      cp2 += distance_function(xe3_evm) /3
